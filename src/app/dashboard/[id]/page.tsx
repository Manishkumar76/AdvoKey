  ...
  useEffect(() => {
    const fetchUserAndConsultations = async () => {
      try {
        ...
        setUserData(user); // async update
  
        const res = await axios.get('/api/consultation');
  
        // Use freshly fetched user object here
        const filtered = res.data.data.filter(
          (c: any) => c.client_id?._id === user._id
        );
      ...
      {userData && (
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-6">
          <div className="flex items-center gap-x-4">
            ...
            <button
              onClick={() => {
                route.push(`/dashboard/${userData._id}/settings`);
              }}
              ...
              <tbody>
                {consultations.map((c: any, idx: number) => (
                  <tr key={c._id} className="border-b border-opacity-20 dark:border-gray-300 dark:bg-gray-50">
                    ...
                    <td className="p-3">
                      <p>{c.lawyer_id?.user?.username || 'Unknown'}</p>
                    </td>
                    <td className="p-3">
